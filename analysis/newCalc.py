...
import numpy as np
# numpy doesnt have quaternion support natively
...
class LaserDataPoint():
    ...
    def __init__(self, zeroVec, xVec, yVec, num):
        ...
        self.hom_registration2measurement = buildHomMatrixfromLaser(
            self.zeroVec, self.xVec, self.yVec)
...
class ViveDataPoint():
    ...
    def __init__(self, pointData, regisPointData, num):
        ...
        self.num = num
        self.hom_measurement2Lighthouse = self.processDataIntoHomogenMatrix(
            self.pointData)
        self.hom_registration2Lighthouse = self.processDataIntoHomogenMatrix(
            self.regisPointData)

        self.hom_regis2Measurment = invertHomMatrix(
            self.hom_measurement2Lighthouse)@self.hom_registration2Lighthouse
...
class CollectionDataPairs():
    ...
    def getListHomMatrixLaser_ViveRef(self):
        ...
        for i in self.listDataPairs:
            listHomLaser.append(
                temp@i.laserDataPoint.hom_registration2measurement@invertHomMatrix(temp))
        return listHomLaser
    ...
    def getListHomMatrixVive_LaserRef(self):
        ...
        temp = calculate_hom_laser2Vive()
        for i in self.listDataPairs:
            listHomViveComplete.append(invertHomMatrix(
                temp)@i.hom_Vive_registration2measurement@temp)
    ...
    def get_distance_acc(self):
        temp_list = list()
        for i in self.listDataPairs:
            temp_list.append(i.distanceAcc)
        return temp_list
...
class DataPair():
    def __init__(self, viveDataPoint, LaserDataPoint, hom_laser2vive):
        ...
        self.hom_Vive_registration2measurement = invertHomMatrix(
            self.hom_laser2vive)@self.viveDataPoint.hom_regis2Measurment@self.hom_laser2vive
        # if self.num==1:
        #     print(self.hom_Vive_registration2measurement)
        #     print(self.laserDataPoint.hom_registration2measurement)
        self.distanceAcc = self.pointAccuracyDistance(
            self.hom_Vive_registration2measurement, self.laserDataPoint.hom_registration2measurement)
...
def makeHomogenousMatrix(matrix, vec):
    ...
    try:
        # if it isnt a scipy.rotation an error will be thrown
...
def reasonable(laserData):
    ...
    # total std of the measurement is handed over by the laser
    if max(laserData[:, 6]) > const.THRESHOLD_FOR_EXCLUSION:
        logging.warning(
            'laserThreshold exceeded. Dataset will not be considered')
    ...
    # yVector in base system
    base_y_vector = T@np.append(baseSys[2, :], [1], 0)
    # zeroVector in base system
    base_zero_vector = T@np.append(baseSys[0, :], [1], 0)
    ...
    for i in range(0, int(numRows/4)):
        ...
        if zeroDiff > const.THRESHOLD_FOR_EXCLUSION:
            logging.warning(
                f"Measurement {i} Deviation too large. The error is {zeroDiff}")
...
def calculate_hom_laser2Vive():
    ...
    """
    vivePosition = np.array(
        [62, 62, -const.TARGET_HOLDER_OFFSET])  # in laser system
    ...
    hom_laser2Vive = makeHomogenousMatrix(
        rot_laser2Vive, -rot_laser2Vive@vivePosition)
    # print(hom_laser2Vive)
    return hom_laser2Vive


def calculate_hom_vive2laser():
    """calculate homogenous matrix from vive tracker to laser target holder KOS 
        as we transforming into the laser system the translation part of the hom
        matrix is known from cosntructional design parameters.
    ...
    vivePosition = np.array(
        [62, 62, -const.TARGET_HOLDER_OFFSET])  # in laser system
    # if const.COORDINATE_SYSTEM == "BLACK":
    # rot_vive2laser = np.matrix(const.LASER2VIVE_ROT_MATRIX_BLACK)
    if const.COORDINATE_SYSTEM == "RED":
        rot_vive2laser = np.matrix(const.VIVE2LASER_ROT_MATRIX_RED)
    hom_vive2laser = makeHomogenousMatrix(
        rot_vive2laser, vivePosition)
    return hom_vive2laser
...
def main():
    ...
    date = const.DATE
    logging.info(
        f"Considering Data from Experiment {const.EXPERIMENT_NUMBER} on {const.DATE}")
    ...
    #####READ in Vive Data####
    viveData_regisPoint = readInData.getRegistrationPointData(
        date, experimentNumber)
    viveData_measurePoints = readInData.getMeasurementPointData(
        date, experimentNumber)
